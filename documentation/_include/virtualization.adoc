...
The system needs configured with access to the following repos:
...
Some additional simple virtual machine management commands
  ...
  * *virsh start <vm-name>* starts a virtual machine by name 
  * *virsh shutdown <vm-name>* performs a soft shutdown of the virtual machine
  * *virsh destroy <vm-name>* performs an abrubt stop of the virtual machine (unplugs the virtual power cord)
