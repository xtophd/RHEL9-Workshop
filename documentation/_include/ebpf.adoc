...
This should line up almost 1-to-1 with the previous output from strace.



=== bpftrace: count specific syscall 

Now we are going to limit the probe to only 'sys_enter_openat' thereby only setting 1 probe and collecting a count.
...
For this last exercise we will utilize both terminal sessions again.  The objective is to set a probe and identify any process that opens our targeted file.
...
[{format_cmd_exec}]
----
bpftrace -e 'tracepoint:syscalls:sys_enter_openat { if (str(args->filename) == "/dev/null") { printf( "%s %s\n", comm, str(args->filename)); } } '
----
...
[{format_cmd_exec2}]
----
*ssh node2 "cat /dev/null"*
----

[{format_cmd_exec2}]
----
*ssh node2 "more /dev/null"*
----

[{format_cmd_exec2}]
----
*ssh node2 "grep something /dev/null"*
----
