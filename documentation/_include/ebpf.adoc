...
The command we will evaluate is `cat /dev/null` and we will use strace to list the system calls
made and with what frequency (count).  Using /dev/null as our target file is simply a means to 
displaying an empty file and not cluttering our output.
...
[{format_cmd_exec}]
----
bpftrace -e 'tracepoint:syscalls:sys_enter_* { if (comm == "cat") { @[probe] = count(); } }' -c "cat /dev/null"
----
...
[{format_cmd_exec}]
----
bpftrace -e 'tracepoint:syscalls:sys_enter_openat { if (comm == "cat") { @[probe] = count(); } }' -c "cat /dev/null"
----
...
The objective is to set a probe and identify any process that opens our targeted file.  The target file in this case
will be a non-existent file, reason being that /dev/null is accessed quite a bit by normal system activity and we
want our probe to stay active for a longer period of time.

So, we will use a file "/tmp/does-not-exist" as our activity target.
...
[{format_cmd_exec}]
----
bpftrace -e 'tracepoint:syscalls:sys_enter_openat { if (str(args->filename) == "/tmp/does-not-exist") { printf( "%s %s\n", comm, str(args->filename)); }}'
----
...
[{format_cmd_exec2}]
----
*ssh node2 "cat /tmp/does-not-exist"*
----

[{format_cmd_exec2}]
----
*ssh node2 "more /tmp/does-not-exist"*
----

[{format_cmd_exec2}]
----
*ssh node2 "grep something /tmp/does-not-exist"*
----
