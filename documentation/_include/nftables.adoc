...
For this unit, we need to ensure firewalld is disabled.
...
Base on specified criteria, *rules* take action on network packets (ie: accept, drop, forward).
...
If the packet matches all of the expressions in the rule, then rule's *statements* are executed
